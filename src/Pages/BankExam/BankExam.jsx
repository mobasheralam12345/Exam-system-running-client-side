...
const YearSelector = () => {
  ...
  const handleProceed = () => {
    ...
    setLoading(true);
    fetch(`${BACKEND_URL}/bank-questions/year/${selectedYear}`)
      .then((res) => res.json())
      .then((data) => {
        setLoading(false);
        if (data.success) {
      ...
      .catch((err) => {
        setLoading(false);
        Swal.fire(`Failed to load exam: Failed to load`);
      });
  };
      ...
      <div className="container mx-auto max-w-4xl relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-slate-600 dark:text-white max-w-2xl mx-auto">
            Select an exam year and start practicing with comprehensive question sets
          </p>
        </div>
        ...
        <div className="bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl rounded-3xl p-8 md:p-12 shadow-2xl border border-slate-200 dark:border-gray-700">
          ...
          <div className="space-y-6">
            <div>
              <label
                htmlFor="yearSelect"
                className="block text-lg font-semibold text-slate-700 dark:text-white mb-3"
              >
                Choose a year:
              </label>
              <select
          ...
          <div className="mt-8 p-4 bg-blue-50 dark:bg-gray-800 rounded-xl border border-blue-100 dark:border-gray-700">
            <p className="text-sm text-slate-600 dark:text-white">
              <strong className="text-blue-600 dark:text-blue-400">Note:</strong> Select your desired exam year from the dropdown above and click "Start Exam" to begin your practice session.
            </p>
          </div>
        </div>
      </div>
