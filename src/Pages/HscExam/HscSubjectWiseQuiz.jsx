...
const HscSubjectWiseQuiz = ({ subject, totalQuestions }) => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://localhost:5000/hsc-questions/hsc-subjectWise/${subject}/${totalQuestions}`);
...
const handleSubmit = async () => {
    ...
    const hscSubject = localStorage.getItem('hscSubject');
...
console.log(dataToSends);
    try {
        console.log("before data send", dataToSends);
        await axios.post('http://localhost:5000/hsc-questions/save-hsc-exam-history', dataToSends);
        console.log("Result saved successfully!", dataToSends);
    } catch (error) {
