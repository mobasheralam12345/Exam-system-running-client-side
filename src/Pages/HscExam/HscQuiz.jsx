...
const HscQuiz = ({ group, board, examYear }) => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      setLoading(true);
      try {
        console.log("Fetching questions with:", group, board, examYear);
        const response = await axios.get(
          `http://localhost:5000/hsc-questions/getPrevHscQuestions/${group}/${board}/${examYear}`
        );
        ...
        if (Array.isArray(subjects) && subjects.length > 0) {
          ...
          setShowResult(false); // Reset this in case of a successful fetch
        } else {
