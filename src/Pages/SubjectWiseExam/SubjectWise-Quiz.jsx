...
const SubjectWiseQuiz = ({ subject, totalQuestions }) => {
    ...
    const fetchQuestions = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:5000/bcs-questions/get-questions/${subject}/${totalQuestions}`
        );
        if (
          response.data &&
  ...
  useEffect(() => {
    if (showResult) {
      const savedOptions = localStorage.getItem("selectedBcsOptions");
      if (savedOptions) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/bcs-questions/save-subject-wise-history",
        dataToSend
      );
